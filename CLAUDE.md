...
- **Rich items** (see `docs/ITEMS.md`): weapons carry `magicBonus` (+X hit & dmg), `toHitBonus`/`damageBonus`/`critRange`, `riderDamage[]` (extra typed dice per hit — applied via `applyRiderDamage`, each type through its own defenses, dice double on crit), `onHitSave{ability,dc,condition,duration}` (→ existing `promptSave` save-or-condition path), `vsType`. Passive equip (`equipmentMods(pc)`): `acBonus`/`statBonuses`/`speedBonus`/`saveBonus`/`allAttackBonus`/`allDamageBonus`/`grantsExtraAttack`/`grantResist/Immune/Vuln`/`conditionImmune`/`grantAction`/`charges`. **`weaponToAction` (DM) and `pvEffectiveActions` (player-view) both emit these fields onto the action** — the player-sent `req.action` is what `resolveCombatAction` resolves, so keep the two in sync. `hydrateWeaponStats` fills base dice from the name; `migratePartyWeapons` fixes old saves on load. Charges refill in `restoreItemCharges` (short/long rest). Builder UI: `editItemEffects`/`saveItemEffects` + `ITEM_LIBRARY` picker (`openItemLibrary`/`applyLibraryItem`) in party.js.
