...
| `js/data/item-library.js` | `ITEM_LIBRARY` — prebuilt magic items for the 📚 picker + living examples of every item field. See `docs/ITEMS.md` for the full field reference |
...
- Player snapshot: `playerView/{uid}` — written by `doCloudWrite` with the **pvSnap**: `party`, `combatants`, `currentRound/currentTurn/combatActive`, `mapState` (null when `hiddenFromPlayers`), `worldMap`, `partyInventory`, `pvMessages`, `pvPartyMessage`, `actionFeed`, `pendingReaction`, `pendingSmite`, `pendingSave`, `pendingCheck`, `lastRejection`, `recentLog` (25).
- Player requests: `playerActions/{uid}/requests` docs → `processPlayerAction(req)` dispatcher. Types: `move, action, castSpell, endTurn, addCharacter, deathSave, toggleFeature, equipItem, journal, useItem, reaction, smite, inspire, standUp, help, ready, saveRoll, skillCheck, damageRoll, recategorize, dropItem, tradeOffer, tradeAccept, tradeDecline, useInspiration`.
- Trading (`js/trade.js`): `shops` (merchant inventories, open ones sync in pvSnap) + `trades` (offers). Player builds a cart → `tradeOffer` → DM inbox `acceptTrade`/`declineTrade` → `executeTrade` (atomic gold+item swap). DM push: `dmPushOffer` → status `awaiting-player` → player `tradeAccept` auto-executes. `itemValue`/`pvItemVal` price trade-ins × shop buyback. After processing a batch, `cloudSaveNow()` flushes so player latency stays ~1s.
...
- **Rich items** (see `docs/ITEMS.md`): weapons carry `magicBonus` (+X hit & dmg), `toHitBonus`/`damageBonus`/`critRange`, `riderDamage[]` (extra typed dice per hit — applied via `applyRiderDamage`, each type through its own defenses, dice double on crit), `onHitSave{ability,dc,condition,duration}` (→ existing `promptSave` save-or-condition path), `vsType`. Passive equip (`equipmentMods(pc)`): `acBonus`/`statBonuses` (additive)/`statSet` (sets a score, highest wins — Gauntlets of Ogre Power → STR 19, applied in `effectiveAbility`)/`speedBonus` (applied in `combatantSpeedFt`)/`saveBonus` (in `saveBonusFor`)/`allAttackBonus`/`allDamageBonus`/`skillBonus` (in `checkBonusFor` + `buildSkillsCardHTML`)/`grantsExtraAttack` (in `attacksPerActionFor`)/`grantResist/Immune/Vuln`/`conditionImmune`/`grantAction`/`grantSpell`/`charges`. **`weaponToAction` (DM) and `pvEffectiveActions` (player-view) both emit these fields onto the action** — the player-sent `req.action` is what `resolveCombatAction` resolves, so keep the two in sync. Item-granted actions/spells (`itemGrantedActions`) carry `sourceItemId`; using one calls `spendItemCharge` (blocks at 0). `hydrateWeaponStats` fills base dice from the name; `migratePartyWeapons` fixes old saves on load. Charges refill in `restoreItemCharges` (short/long rest). Builder UI: `editItemEffects`/`saveItemEffects` + `ITEM_LIBRARY` picker (`openItemLibrary`/`applyLibraryItem`) in party.js.
- **Ability/skill checks** (`promptCheck(target, skillOrAbilityKey, dc, meta)`): mirrors `promptSave` — a player ally rolls on their phone (`pendingCheck` → `skillCheck` req → `processPlayerSkillCheck` → `resolveCheckOutcome`), a monster rolls in the DM modal. `checkBonusFor` = ability mod (with item stat bonuses) + prof × skill rank (`pc.skills[key]` 0/1/2) + item `skillBonus`. DM entry: 🎲 button on ally combatant rows (`dmRequestCheck`/`dmFireCheck`).
